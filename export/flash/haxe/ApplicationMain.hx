...
class ApplicationMain {
	...
	static var mPreloader:inscrutablegames.Preloader;
	...
	public static function main() {
		...
		if (loaded < total || true) /* Always wait for event */ {
			call_real = false;
			mPreloader = new inscrutablegames.Preloader();
	...
	private static function begin() {
		...
		for (methodName in Type.getClassFields(game.Main))
		{
			if (methodName == "main")
			{
				hasMain = true;
		...
		if (hasMain)
		{
			Reflect.callMethod(game.Main, Reflect.field (game.Main, "main"), []);
		}
		else
		{
...
class DocumentClass extends game.Main { }
